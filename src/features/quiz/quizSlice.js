...
export const quizSlice = createSlice({
  ...
  reducers: {
    addQuiz: (state, action) => {
      ...
      const newId = state.quizzes.questions_answers.length > 0 
        ? Math.max(...state.quizzes.questions_answers.map(q => q.id)) + 1 
        : 1;
      newQuestion.id = newId;
      state.quizzes.questions_answers.push(newQuestion);
      state.quizzes.modified = new Date().toISOString();
    },

    updateQuiz: (state, action) => {
      const { quizId, updatedQuiz } = action.payload;
      const index = state.quizzes.questions_answers.findIndex(q => q.id === quizId);
      if (index !== -1) {
        state.quizzes.questions_answers[index] = updatedQuiz;
        state.quizzes.modified = new Date().toISOString();
      } else {
        console.warn("Quiz not found for update:", quizId);
      }
    },

    updateAnswerId: (state, action) => {
      const { questionId, answerId } = action.payload;
      const question = state.quizzes.questions_answers.find(q => q.id === questionId);
      if (question) {
    ...
    updateScore: (state, action) => {
      state.quizzes.score = action.payload.score;
    },
  },
});

export const { addQuiz, updateQuiz, updateAnswerId, updateScore} = quizSlice.actions;
